...
import { createWorkflow, createStep, Telemetry } from '@mastra/core';
...
async function getWorkflowHistory() {
  ...
  const recentRuns = await storage.getWorkflowRuns({
    fromDate: new Date(Date.now() - 24 * 60 * 60 * 1e3),
    limit: 10,
    offset: 0
  });
  console.dir(recentRuns, {
    depth: null
  });
